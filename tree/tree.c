...
Node* find(Node* r, char* name){// tim node co ten la name o trong cay co goc la r
  if(r == NULL) return NULL;
  if(strcmp(r->name,name) == 0) return r;
  Node* p = r->leftMostChild;
  while(p != NULL){
    Node* q = find(p,name);// ham de quy
    if(q != NULL) return q;
    p = p->rightSibling;
  }
}
...
void processHeight(){ 
  ...
  if(p == NULL) printf("Not Found %s\n",name);
    ...
    printf("Found %s having height = %d\n",name,height(p));// name là con thứ mấy của người cha trên.
  }
}
...
int main(){
  ...
  while(1){
	...
	__fpurge(stdin);
	printf("Enter child: "); scanf("%s",child);
	addChild(name, child) ;
      }
    ...
    //else if(strcmp(cmd,"AddChild")==0) addChild();
